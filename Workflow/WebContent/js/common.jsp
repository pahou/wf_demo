...
<!-- 上传 start -->
<!-- 首先需要引入plupload的源代码 -->
<script type="text/javascript" src="<%=contextPath %>/js/plupload/js/plupload.full.js"></script>
<script type="text/javascript" src="<%=contextPath %>/js/plupload/js/jquery.plupload.queue/jquery.plupload.queue.js"></script>
<script type="text/javascript" src="<%=contextPath %>/js/plupload/js/i18n/cn.js"></script>
<link rel="stylesheet" href="<%=contextPath %>/js/plupload/js/jquery.plupload.queue/css/jquery.plupload.queue.css">
<!-- 上传 end -->

<!-- 视频播放  start -->
<link href="<%=contextPath %>/js/video-js/video-js.css" rel="stylesheet" type="text/css">
<script type="text/javascript" src="<%=contextPath %>/js/video-js/video.js"></script>
<!-- 视频播放  end -->
...
var id = '<s:property value="id"/>';//记录当前选中ID
...
var lastIndex;
 $(function(){
	...
	if ("true" == hasTree) {
			...
			var treesjson = eval("(" + treesInfo + ")");//双重括号，eval把字符串转化成对象，单括号，eval把字符串当成js执行
			for (var i = 0; i < treesjson.length; i++) {//循环每一棵树
				var name = treesjson[i].name;//每棵树的id（name）都不一样,用于构造前台的树，
		...
		if ("true" == hasForm) {
			...
			for (var i = 0; i < fjson.length; i++) {
				...
				if (!lazy) {//判断是否延迟加载
					...
					var showText = fjson[i].showText;
					//var root = fjson[i].root;
					createHiddenInfo(fid, JSON.stringify(fields));//隐藏域保存form属性
					createForm(span, fid, method, fields, dataUrl,null,null,lazy,text,showText);//创建form
		...
		if ("true" == hasToolBar) {
			...
			var toolBarjson = eval("(" + toolBarInfo + ")");
			 for (var i = 0; i < toolBarjson.length; i++) {
				 crateToolBar(toolBarjson[i].buttons);//创建toolbar
			} 
		}
...
/* 		 $("#index_body").layout("onResize",function(width, height){
			...
			/* 			error : function(XMLHttpRequest, textStatus, errorThrown) {
					    ...
					    complete:function(XMLHttpRequest,textStatus){  
					        ...
					        if(resText=='timeout'){    
					        	...
					        	setTimeout("getTopWinow().location.href = 'http://150.84.11.47:8080/zkpt/';",
									3000);
	...
	function longWidth2(spanId) { 
		return ($('#'+spanId).parent()[0].clientWidth)*0.75;
		//return (document.all.userRight.clientWidth+3)*0.75;
	};
	function longWidth3(spanId) { 
		return ($('#'+spanId).parent()[0].clientWidth-31 )*0.75;
		//return (document.all.userRight.clientWidth+3)*0.75;
	};
	...
	function addFormStyle(spanId){
		var fixWidth_rolev = fixWidth_role(spanId);
		var longWidthv = longWidth(spanId);
		var fixWidth_rolecombov = fixWidth_rolecombo(spanId);
		var nameWidthv = nameWidth(spanId);
		var longWidth2v = longWidth2(spanId);
		var longWidth3v = longWidth3(spanId);
		//太小时不调整样式
		if(fixWidth_rolev>10){
			$('#'+spanId+' .docform .excombo').combo({ width:fixWidth_rolev+2});
			$('#'+spanId+' .divtable .tabname').css({'width':fixWidth_rolev+'px','text-align':'right','background':'#f9f9f9','border-right':'1px solid #d3d3d3','padding':'5px 6px 5px 5px'});
			$('#'+spanId+' .divtable .tabint').css({'width':fixWidth_rolev+'px','padding':'5px 6px 5px 5px'});
			$('#'+spanId+' .divtable .tabint input').css({'width':fixWidth_rolev+'px','color':'#000','font-family':'微软雅黑','font-size':'12px'});
		}
		if(longWidthv>10){
			$('#'+spanId+' .longtable .longint').css({'width':longWidthv+'px','height': '20px','padding':'5px 6px 5px 5px' });
			$('#'+spanId+' .areatable .tabtext').css({'width':longWidthv+'px','padding':'5px 4px 5px 5px'});
			$('#'+spanId+' .areatable .tabtext textarea').css({'width':longWidthv+'px','color':'#000','border':'1px solid #d3d3d3','font-family':'微软雅黑','font-size':'12px','resize':'none'});

		}
		if(fixWidth_rolecombov>10){
			$('#'+spanId+' .divtable .combo .combo-text').css({'width':fixWidth_rolecombov+'px'});
	    	$('#'+spanId+' .divtable .combo .validatebox-text').css({'width':fixWidth_rolecombov+'px'});
		}
		if(nameWidthv>10){
			$('#'+spanId+' .longtable .longname').css({'width':nameWidthv+'px','text-align':'right','background':'#f9f9f9','border-right':'1px solid #d3d3d3','padding':'5px 6px 5px 5px'});
			$('#'+spanId+' .areatable .tabname ').css({'width':nameWidthv+'px','text-align':'right','background':'#f9f9f9','border-right':' 1px solid #d3d3d3','padding':'5px 6px 5px 5px'});

		}
		if(longWidth2v>10){
			$('#'+spanId+' .docform .exlongcombo').combo({ width:longWidth2v});
			$('#'+spanId+' .longtable .longint input ').css({'width':longWidth2v+'px','color':'#000','font-family':'微软雅黑','font-size':'12px'});
			$('#'+spanId+' .longtable .combo').css({'width':longWidth2v+'px','height':'20px'});
		}
		if(longWidth3v>10){
			$('#'+spanId+' .longtable .combo .combo-text').css({'width':longWidth3v+'px','height':'20px'});
			$('#'+spanId+' .longtable .combo .validatebox-text').css({'width':longWidth3v+'px','height':'20px'});
		}
	...
	function initProcessViewBean(){
		window.processViewBean = {};
		processViewBean.userId = '<s:property value="userId"/>';
		processViewBean.systemName = '<s:property value="systemName"/>';
		processViewBean.ccbh = '<s:property value="ccbh"/>';
	}
	
	//设置页面标题
	function initPageTitle(){
		var title = '<s:property value="pageTitle"/>';
